...
class ModelConfig:
    ...
    @classmethod
    def llama3_2_1b(cls):  # meta-llama/Llama-3.2-1B
        # These are estimations based on typical Llama architectures and aiming for ~1B parameters.
        # Exact internal Meta parameters for 1B might not be publicly disclosed in detail.
        return cls(
            num_layers=24,  # A common number of layers for smaller models
            vocab_size=128256,
            embed_dim=2048,
            num_heads=32,
            num_kv_heads=8,
            head_dim=2048 // 32,  # embed_dim // num_heads
            hidden_dim=4 * 2048,  # 4 * embed_dim
            norm_eps=1e-05,
            rope_theta=500000.0,
        )

    @classmethod
    def llama3_2_3b_instruct(cls):
        return cls(
            num_layers=24,  # Reduced from 32 for a smaller model
            vocab_size=128256,  # Likely remains the same as Llama 3
            # ValueError: shape must match for model.embed_tokens.weight, got (128256, 3072) vs (128256, 2560)
            embed_dim=3072,  # Scaled down from 4096 (e.g., roughly proportional to sqrt(3B/8B) * 4096, or a common smaller dimension)
            # ValueError: shape must match for model.layers.0.mlp.down_proj.weight, got (8192, 3072) vs (8960, 3072)
            hidden_dim=8192,  # Scaled down from 14336 (maintaining roughly the same multiplier relative to embed_dim)
            # RuntimeError: Failed to transform tensor model.layers.0.self_attn.k_proj.weight with shape (3072, 1024): cannot reshape array of shape (3072, 1024) (size 3145728) into shape (3072, 5, 128) (size 1966080)
            # Or, if the original model had num_heads = 8 and head_dim = 128 (and no GQA, so num_kv_heads == num_heads), then 8 * 128 = 1024.
            num_heads=24,  # Scaled down from 32
            head_dim=128,  # Often kept consistent, or slightly reduced
            num_kv_heads=8,  # Scaled down from 8, maintaining GQA/MQA structure
            norm_eps=1e-05,  # Typically remains constant
            rope_theta=500_000,  # Likely remains consistent with Llama 3 for context length
        )
...
"""
      return cls(
          ...
          # RuntimeError: Failed to transform tensor model.layers.0.self_attn.k_proj.weight with shape (3072, 1024): cannot reshape array of shape (3072, 1024) (size 3145728) into shape (3072, 5, 128) (size 1966080)
          # Or, if the original model had num_heads = 8 and head_dim = 128 (and no GQA, so num_kv_heads == num_heads), then 8 * 128 = 1024.
          num_heads=20,     # Scaled down from 32
          head_dim=128,     # Often kept consistent, or slightly reduced
          num_kv_heads=5,   # Scaled down from 8, maintaining GQA/MQA structure
          norm_eps=1e-05,   # Typically remains constant
          rope_theta=500_000, # Likely remains consistent with Llama 3 for context length
      )
"""
...
class Einsum(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.shape = shape
        self.w = nnx.Param(nnx.initializers.normal()(rngs.params(), shape), sharding=sharding)
...
class Attention(nnx.Module):
    ...
    def __call__(
        ...
        attn = jax.nn.softmax(attn.astype(jnp.float32), axis=-1).astype(key_proj.dtype)
...
class Llama3(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.layers = [DecoderLayer(config=config, rngs=rngs, shd_config=shd_config) for _ in range(config.num_layers)]
