...
def init_batched_beam_state(
    ...
    return BeamSearchSamplingState(
        ...
        "logits_buffer": (jnp.repeat(logits_buffer, beam_size, axis=0) if logits_buffer is not None else None),
    }
...
def beam_search_step(
    ...
    log_probs = jax.nn.log_softmax(logits, axis=-1)  # (batch_size, beam_size, vocab_size)
    if not state.initialized:
        # the beam information is duplicated, just select the first one from every
        # batch to dedup.
        candidate_scores_per_item = log_probs[:, 0, :].reshape(batch_size, vocab_size)
    else:
        # Add current beam scores to the new log probabilities
        candidate_scores = state.scores[:, :, None] + log_probs  # (batch_size, beam_size, vocab_size)
        ...
        candidate_scores_per_item = candidate_scores.reshape(batch_size, beam_size * vocab_size)

    # don't forget finished beams
    scores_of_finished_beams = jnp.where(done.reshape(batch_size, beam_size), state.scores, -jnp.inf)
    # (batch_size, beam_size * vocab_size + beam_size)
    combined_scores = jnp.concatenate([candidate_scores_per_item, scores_of_finished_beams], axis=1)
    ...
    # finished beams.
    is_extension_candidate = top_k_flat_indices < candidate_scores_per_item.shape[-1]
    ...
    next_token_indices = jnp.where(is_extension_candidate, ext_next_token_indices, done_next_token_indices)
    # Construct the new beam search state
    batch_indices = jnp.arange(batch_size)[:, None].repeat(beam_size, axis=1)  # (batch_size, beam_size)
    gather_indices = jnp.stack([batch_indices, source_beam_indices], axis=-1)  # (batch_size, beam_size, 2)
    ...
    source_logits_buffer = (
        gather_beams_by_indices(logits_buffer, gather_indices) if logits_buffer is not None else None
    )  # (batch_size * beam_size, max_total_len, vocab_size)

    source_beams_done = gather_beams_by_indices(done, gather_indices)

    new_cache = jax.tree.map(lambda x: gather_beams_by_indices(x, gather_indices), cache)
...
def finalize_beam_search_state(
    ...
    logits_buffer = logits_buffer.reshape((batch_size, beam_size, -1)) if logits_buffer is not None else None
    return {
        "token_buffer": token_buffer[:, 0, ...],
        "logits_buffer": (logits_buffer[:, 0, ...] if logits_buffer is not None else None),
    }
