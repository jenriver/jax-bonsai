...
def _init_cache(
    ...
    def _init_layer_cache() -> LayerCache:
        return {
            "k": jnp.zeros((batch_size, cache_size, num_kv_heads, head_dim), dtype=dtype),
            "v": jnp.zeros((batch_size, cache_size, num_kv_heads, head_dim), dtype=dtype),
...
class Sampler:
    ...
    def transformer(self) -> nnx.Module:
        return nnx.merge(self._transformer_graphdef, self._flattened_transformer_state)
    ...
    def transformer_state(self, state: statelib.State) -> None:
        ...
        def check_tree_structure(tree1, tree2):
            ...
            def check_shape_dtype_sharding(x, y):
                return jnp.shape(x) == jnp.shape(y) and jnp.dtype(x) == jnp.dtype(y) and x.sharding == y.sharding

            if not all(jax.tree_util.tree_leaves(jax.tree_util.tree_map(check_shape_dtype_sharding, tree1, tree2))):
                raise ValueError(
                    f"New state must have the same shape, dtype and sharding as the old state. {tree1} vs {tree2}"
                )
    ...
    def init_sample_state(
        ...
        input_mask = input_mask.at[:, :num_input_tokens].set(all_input_ids != self.tokenizer.pad_id())
    ...
    def _prefill_fn(self, params: statelib.State, sampler_state: _SamplingState) -> _SamplingState:
        """Performs prefill."""
        batch_size = sampler_state.token_buffer.shape[0]

        tokens = jax.lax.dynamic_slice(
            sampler_state.token_buffer,
            start_indices=jnp.zeros((sampler_state.token_buffer.ndim,), dtype=jnp.int32),
    ...
    def _decode_fn(
        ...
        def cond_fn(sampler_state: _SamplingState):
            return (sampler_state.decoding_step < sampler_state.total_sampling_steps) & jnp.any(
                jnp.logical_not(sampler_state.done)
            )

        return jax.lax.while_loop(cond_fn, sample_with_params, sampling_state)

    def _sample_step(self, params: statelib.State, sampler_state: _SamplingState) -> _SamplingState:
        ...
        if updated_sampler_state.logits_buffer is not None:
            ...
            logits_buffer = updated_sampler_state.logits_buffer.at[:, decoding_step + 1].set(next_logits)
        else:
            logits_buffer = None
    ...
    def __call__(
        ...
        sampling_state = self._compiled_decode_fn(self._flattened_transformer_state, sampling_state)
        ...
        for i, token_buffer in enumerate(token_buffers):
            start_idx = (
                utils.find_first_non_pad_idx(token_buffer, self.tokenizer.pad_id()) if echo else max_prompt_length
            )
            end_idx = (
                utils.find_first_eos_idx(token_buffer[max_prompt_length:], self.tokenizer.eos_id()) + max_prompt_length
            )
            out_tokens.append(token_buffer[start_idx:end_idx])
            if return_logits:
                out_logits.append(logits_buffers[i][start_idx:end_idx])

        decoded_outputs = [self.tokenizer.decode(tokens.tolist()) for tokens in out_tokens]
